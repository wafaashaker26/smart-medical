...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  _filtered = _allSymptoms.where((s) => s.contains(q.toLowerCase())).toList());
  ...
  Widget _tabBar() => Container(
    ...
    child: TabBar(
      ...
      tabs: [
        ...
        Tab(text: _chatHistory.length > 1 ? 'Consult •' : 'Consult'),
      ],
    ),
  );
  ...
  Widget _resultsTab() {
    if (_loading) return const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      SizedBox(height: 14),
      Text('Analyzing...', style: TextStyle(color: kMuted, fontSize: 13)),
    ]));

    if (_result == null) return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Container(
        ...
        child: const Icon(Icons.bar_chart_rounded, color: kMuted, size: 40),
      ),
      const SizedBox(height: 16),
      const Text('No results yet', style: TextStyle(color: kText, fontSize: 14, fontWeight: FontWeight.w500)),
      const SizedBox(height: 4),
      const Text('Select symptoms and run analysis', style: TextStyle(color: kMuted, fontSize: 12)),
    ]));

    final r      = _result!;
    final rColor = r.riskLevel == 'High' ? kRed : r.riskLevel == 'Medium' ? kYellow : kGreen;
    final rBg    = r.riskLevel == 'High' ? kRedL : r.riskLevel == 'Medium' ? kYellowL : kGreenL;
    final rBd    = r.riskLevel == 'High' ? kRedB : r.riskLevel == 'Medium' ? kYellowB : kGreenB;
    ...
    return ListView(
      ...
      children: [
        ...
        _card(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _sectionLabel('Primary diagnosis'),
          const SizedBox(height: 10),
          Text(r.disease, style: const TextStyle(color: kText, fontSize: 20, fontWeight: FontWeight.w600)),
          const SizedBox(height: 10),
          Row(children: [
            Container(
              ...
              child: Text(
                '${r.riskLevel} risk',
                style: TextStyle(color: rColor, fontSize: 11, fontWeight: FontWeight.w500),
              ),
            ),
            const SizedBox(width: 10),
            Text('Score: ${r.riskScore}', style: const TextStyle(color: kMuted, fontSize: 12)),
          ]),
        ])),
        const SizedBox(height: 10),
        ...
        // Symptom analysis
        _card(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _sectionLabel('Symptom analysis'),
          const SizedBox(height: 10),
          ...r.explanation.map((e) => Padding(
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(width: 10),
              Expanded(child: Text(e, style: const TextStyle(color: kText, fontSize: 13, height: 1.5))),
            ]),
          )),
        ])),
        const SizedBox(height: 10),
        ...
        OutlinedButton(
          ...
          child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(Icons.chat_bubble_outline_rounded, size: 16, color: kMuted),
            SizedBox(width: 8),
            Text('Continue to consult', style: TextStyle(color: kMuted, fontSize: 13)),
          ]),
        ),
      ],
    );
